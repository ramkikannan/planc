...
To this end, we adopt hypergraph partitioning techniques used in parallel sparse matrix literature to our case~\cite{caay:99,aycu:08}.
...
Traditional checkerboard hypergraph partitioning aims to partition the matrix $\Am$ into $P_r$ row slices first, and $P_c$ column slices next to obtain an $P_r \times P_c$ checkerboard partition.
...
In the context of NMF, one potential issue arises when the matrix has a large variance in the numbe r of nonzeros in its rows/columns, which in turns yields unbalanced row/column strides.
This in turn creates an imbalance in the NLS computations as rows/cols of $\HH$/$\WW$ are distributed to the processes in the same stride.
To alleviate this issue, we modify this scheme slightly, which we call \textbf{CH2}.
...
This partitioning model succesfully grasps the computation~(both SpMM and NLS) and communication requirements using checkerboard topology for sparse NMF, yet it is very costly to compute in practice due to high number of constraints in the row-net hypergraph.
