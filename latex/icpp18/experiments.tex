...
\addplot  table [x={edgefactor}, y={collective}] {data/nnzsweepall_k48_p4096_comp.dat};
\addplot  table [x={edgefactor}, y={cp1d}] {data/nnzsweepall_k48_p4096_comp.dat};
\addplot  table [x={edgefactor}, y={cp2d}] {data/nnzsweepall_k48_p4096_comp.dat};
\addplot  table [x={edgefactor}, y={cr}] {data/nnzsweepall_k48_p4096_comp.dat};
\addplot  table [x={edgefactor}, y={cu}] {data/nnzsweepall_k48_p4096_comp.dat};
%\legend{collective, cp1d, cp2d, cr, cu}
\end{axis}
\end{tikzpicture}
\subcaption{Computation cost for low rank $k$=48 on 4096 procs}
...
\addplot  table [x={edgefactor}, y={collective}] {data/nnzsweepall_k48_p4096_comm.dat};
\addplot  table [x={edgefactor}, y={cp1d}] {data/nnzsweepall_k48_p4096_comm.dat};
\addplot  table [x={edgefactor}, y={cp2d}] {data/nnzsweepall_k48_p4096_comm.dat};
\addplot  table [x={edgefactor}, y={cr}] {data/nnzsweepall_k48_p4096_comm.dat};
\addplot  table [x={edgefactor}, y={cu}] {data/nnzsweepall_k48_p4096_comm.dat};
\legend{collective, cp1d, cp2d, cr, cu}
\end{axis}
\end{tikzpicture}
\subcaption{Communication cost for low rank $k$=48 on 4096 procs}
...
%\renewcommand{\numiterations}{30}
%\renewcommand{\datafile}{data/nnzsweepall_k48_p4096_comm.dat}
%\speedup
%\end{tikzpicture}
%\subcaption{Communication cost for low rank $k$=48 on 4096 procs}
...
%\renewcommand{\minvalue}{1}
%\renewcommand{\numiterations}{30}
%\renewcommand{\datafile}{data/nnzsweepall_k48_p4096_comp.dat}
%\speedup
%\end{tikzpicture}
%\subcaption{Communication cost for low rank $k$=48 on 4096 procs}
...
\speedup
\end{tikzpicture}
\subcaption{Flickr dataset low rank $k=48$ on Rhea}
\label{fig:flickr-speedup-48}
\end{subfigure}
...
\speedup
\end{tikzpicture}
\subcaption{Delicious dataset low rank $k=48$ on Rhea}
...
\speedup
\end{tikzpicture}
\subcaption{Delicious dataset low rank $k=48$ on Bluegene/Q}
...
As the number of processes increases to 3072, the communication time of \crp\ and \cpp\ stays reasonably low, whereas in the case of \unp\  and \urp, we clearly observe that the communication cost dominates the execution time using both low rank values $k=16$ and $k=48$.
...
\makeplot
\end{tikzpicture}
\subcaption{Rhea $p=16$, Low Rank $k=48$}
\end{subfigure}
~
\begin{subfigure}[t]{0.25\textwidth}
\begin{tikzpicture}
...
\makeplot
\end{tikzpicture}
\subcaption{Rhea $p=3072$, Low Rank $k=48$}
\end{subfigure}
...
\caption{Time Split up of Flickr and Delicious for Low Rank $k=48$ on Bluegene/Q. The left two bars are for the Flickr matrix, and the right two bars are for the Delicious matrix}
