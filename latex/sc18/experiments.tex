...
\red{Our algorithm (with the dimension tree optimization) achieves a speedup of $1771\times$ on 4096 cores over the same implementation running on 1 core.}
...
\strongscalingplot
\end{tikzpicture}
\caption{Strong scaling of 3D synthetic tensor with dimension $1024\times 1024\times 1024$ on processor grids $2^k\times 2^k\times 2^k$ for \red{$k\in\{0,1,2,3,4\}$}.  The rank is fixed at 32.}
...
\subsection{\red{Varying Processor Grid}}

\red{In order to illustrate the effect of processor grid choice on running time, we show in \Cref{fig:commsweep10} a time breakdown over various processor grid choices for a 4D problem on 81 processors.
...
%\labels
\end{tikzpicture}
\caption{\red{Time breakdown for $243\times243\times243\times243$ tensor and rank $R=10$ on 81 processors for varying processor grids.}}
