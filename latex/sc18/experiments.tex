...
Unlike Liavas \cite{LK+17b} that uses Eigen matrix library, we use Armadillo \cite{sanderson2010} for matrix representation
and operations.  
...
We observe an overall time increase with increased $k$, but each part of the computation scales slightly differently.
The multi-TTV computation ({\em mTTV}) scales linearly with the increasing $k$,  whereas the partial MTTKRP ({\em PM}) is scaling super-linearly. 
This is because mTTV is cast as matrix-vector products (DGEMV) and PM is cast as matrix-matrix products (DGEMM).
As $k$ increases from 10 to 50, DGEMM performance improves but DGEMV performance is constant.  
The local NLS time is increasing with $O(k^3)$ as expected and the All-Reduce required for the Gram matrices scales with $O(k^2)$, becoming a significant cost for larger $k$. 
