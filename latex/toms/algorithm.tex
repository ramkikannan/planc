...
\smaller \NaiveAlg & \smaller $4\frac{mnk}{p}+(m{+}n)k^2+F\lt(\frac mp,\frac np,k\rt)$ & \smaller $O((m+n)k)$ & \smaller $O(\log p)^*$ & \smaller $O\lt(\frac{mn}{p}+(m{+}n)k\rt)$ \\ \hline
\smaller \ParNMF\ ($m/p \geq n$) & \smaller $4\frac{mnk}{p}+\frac{(m+n)k^2}{p} + F\lt(\frac mp,\frac np,k\rt)$ & \smaller $O(nk)$ & \smaller $O(\log p)^*$ & \smaller $O\lt(\frac{mn}{p}+\frac{mk}{p}+nk\rt)$ \\ \hline
\smaller \ParNMF\ ($m/p < n$) & \smaller $4\frac{mnk}{p}+\frac{(m+n)k^2}{p}+F\lt(\frac mp,\frac np,k\rt)$ & \smaller $O\lt( \sqrt{\frac{mnk^2}{p}}\rt)$ & \smaller $O(\log p)^*$ & \smaller $O\lt(\frac{mn}{p}+\sqrt{\frac{mnk^2}{p}}\rt)$ \\ \hline
\smaller Lower Bound & $-$ & \smaller $\Omega\lt(\min\lt\{\sqrt{\frac{mnk^2}{p}},nk\rt\}\rt)$ & \smaller $\Omega(\log p)$ & \smaller $\frac{mn}{p}+\frac{(m+n)k}{p}$ \\ \hline
\end{tabular}
\normalsize
\end{center}
