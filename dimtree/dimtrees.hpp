...
void partial_MTTKRP( Output_Layout OL, int s, direction D, tensor * T, double * A, int r, double * C, int num_threads ){
	...
	/**
		...
		2) trans_tensor, balances dgemm_layout with contracting over left or right modes
		3) trans_A, balanes dgemm_layout with the assumption A is always in row major ordering
		4) m, rows of T->data
		5) r, columns of A
		6) k, common dimensions being contracted over
		7) alpha
		8) T->data, tensor data
		9) k, stride of T->data
		10) A, a row major matrix
		11) r, stride of A
		12) beta
		13) C, output matrix
		14) output_stride	
		print_dgemm_inputs( dgemm_layout, trans_tensor, trans_A, m, r, k, alpha, k, r, beta, output_stride );
	*/
	cblas_dgemm( dgemm_layout, trans_tensor, trans_A, m, r, k, alpha, T->data, tensor_stride, A, r, beta, C, output_stride );
}
...
/**
	...
	Wrapper function for computing a partial_MTTKRP with KRP and outputing a (Row_Major) factor matrix.
	This function should always be used on edges that lead to leaves of the tree.
	D tells you s
...
/**
	...
	Wrapper function for computing partial_MTTKRP with KRP and outputing a tensor.
	This function should be used on edges of the tree that lead to non-leaf nodes.
	1) s, split point
	2) D, direction
	3) input_ktensor, ktnesor whose factor matrices to are used to form the KRP
	4) input_tensor, tensor to contract
	5) output_tensor
	6) num_threads
*/
void partial_MTTKRP_with_KRP_output_T( int s, direction D, ktensor * input_ktensor, tensor * input_tensor, tensor * output_tensor, int num_threads ){
	...
	LR_tensor_Reduction( input_tensor, output_tensor, new_s, opposite_direction(D) );
...
/**
	multi_TTV();
	performs a multi_ttv with between subtensors of T->data and the column of the matrix A.
	Outputs to the matrix/tensor C.
	1) OL, output_Layout
	2) D, direction
	3) s, the split point
	4) T, tensor
	5) A, ttv the columns of this matrix with the tensor
	6) r, columns in A
	7) C, output matrix
	8) num_threads
*/
void multi_TTV( Output_Layout OL, int s, direction D, tensor * T, double * A, int r, double * C, int num_threads ){
	...
	if( r != T->dims[T->nmodes-1] ){	// the last mode of T must be of length r
		printf("r and the last mode of the tensor T must be equal, r = %d, T->dims[T->nmodes-1] = %d\nmulti_TTV()\n\nExit\n", r, T->dims[T->nmodes-1] ); exit(-1);
	}
	...
	for( i = 0; i < s+1; i++ )		// by convenction left product includes s
		left_dims_product *= T->dims[i];
	for( i = s+1; i < T->nmodes-1; i++ )	// -1 because we do not want to include the rank dimension
		right_dims_product *= T->dims[i];

	trans_tensor = CblasNoTrans;		// alwyas NoTrans because dgemv_layout is based on left and right
	if( D == ::direction::left ){
	...
	for( i = 0; i < r; i++ ){
		/**
			...
			print_dgemv_inputs( dgemv_layout, trans_tensor, m, n, alpha, i*m*n, tensor_stride, i, r, beta, output_col_stride, output_stride );
		*/
		cblas_dgemv( dgemv_layout, trans_tensor, m, n, alpha, T->data + i * m * n, tensor_stride, A + i, r, beta, &C[i * output_col_stride], output_stride );
	}
}
...
void multi_TTV_with_KRP( Output_Layout OL, int s, direction D, tensor * T, ktensor * Y, double * C, int num_threads ){
	...
	if( T->nmodes != Y->nmodes+1 ){
		printf("In multi_TTV_with_KRP(), T->nmodes = %d and Y->nmodes = %d\nThe tensor should have 1 more mode than the ktensor.\nExit\n", T->nmodes, Y->nmodes);
		exit(-1);
	}
...
/**
	...
	Wrapper function for performing a multi_TTV with a KRP and outputing a (RowMajor) factor matrix.
	This should be used on edges leading to leave of the tree.
	1) D, direction to contract
	2) input_tensor, tensor to contract
...
void multi_TTV_with_KRP_output_T( int s, direction D, tensor * input_tensor, ktensor * input_ktensor, tensor * output_tensor, int num_threads ){
	...
	if( op_D == ::direction::left ){	// if we went left we didn't get the rank dimension
		output_tensor->nmodes += 1;
		output_tensor->dims[output_tensor->nmodes-1] = input_ktensor->rank;
		output_tensor->dims_product *= input_ktensor->rank;
	}
