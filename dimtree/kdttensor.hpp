#ifndef DIMTREE_TENSORS_HPP_
#define DIMTREE_TENSORS_HPP_
...
void printM_ColMajor(double* M, long int num_cols, long int num_rows){
	...
	for(j = 0; j < num_rows; j++){
		for(i = 0; i < num_cols; i++){
			if(i == 0){printf(",%0.10lf", M[i*num_rows + j] );}
			else{printf(",%0.10lf", M[i*num_rows + j]);}
		}
		printf("\n");
	}	
...
void printM_RowMajor(double* M, long int num_cols, long int num_rows){
	...
	for(j = 0; j < num_rows; j++){
		for(i = 0; i < num_cols; i++){
			if(i == 0){printf(",%0.10lf", M[j*num_cols + i] );}
			else{printf(",%0.10lf", M[j*num_cols + i]);}
		}
		printf("\n");
	}	
...
void update_Partial_Hadamards( ktensor * Y, long int * indexers, double * partial_Hadamards, double ** reordered_Factors ){
	...
	/*
		...
		While the ith place is 0 keep searching, you will always hit a nonzero before the end because
		if the entries are all zero we have either just stared or just ended the KR product.
		update_from will hold the index of the last factor matrix to cycle, the one we should update from
			including that factor matrix.
	*/
	...
	if( update_from == 0 ){
		...
		*/
		for( i = Y->nmodes - 4; i >= 0; i-- ){	// start from the right most partial, Y->nmodes-4 is the last index of the
							// partialHadamards array, i iterates over the partials array
			if( i  == Y->nmodes - 4){	// on the first iteration we need to access the last and second factor matrix
				/*
					call vdMul to get the hadamard product of the last two factor matrices
					1) length of the vectors
					2) second to last factor matrix
					3) last factor matrix
					4) output location in partial_Hadmards
				*/
				vdMul( Y->rank, &reordered_Factors[Y->nmodes-3][Y->rank*indexers[Y->nmodes-3]], &reordered_Factors[Y->nmodes-2][Y->rank*indexers[Y->nmodes-2]], &partial_Hadamards[Y->rank*i] );
			}
			else{	// we are not updating the last partial_Hadamard so we can use a previous partial
				/*
					call vdMul to get the hadamards product of the current factor matrix based off i
					and the partial_Hadamards one stride to the right
				...
				*/
				vdMul( Y->rank, &reordered_Factors[i+1][Y->rank*indexers[i+1]], &partial_Hadamards[Y->rank*(i+1)], &partial_Hadamards[Y->rank*i] );
			}
		}
	}
	else{	// the case that we are updating from somewhere that is update_from < Y->nmodes - 4, not the last or second to last
		for( i = update_from - 1; i >= 0; i-- ){
			/*
				call vdMul to update the partial_Hadamards to the left of and including i
			*/
			vdMul( Y->rank, &reordered_Factors[i+1][Y->rank*indexers[i+1]], &partial_Hadamards[Y->rank*(i+1)], &partial_Hadamards[Y->rank*i] );
		}
	}
...
void KR_RowMajor( ktensor * Y, double * C, long int n ){
	...
	for( i = 0; i < Y->dims[lF]; i++ ){	// loop over the rows of the left matrix
		for( j = 0; j < Y->dims[rF]; j++ ){ // loop over the rows of the right matrix
			vdMul( Y->rank, &Y->factors[lF][i*Y->rank], &Y->factors[rF][j*Y->rank], &C[i*Y->rank*Y->dims[rF] + j*Y->rank] );
		}
	}
}
...
void Multi_KR_RowMajor( ktensor * Y , double * C, long int n ){
	if( Y->nmodes == 3 ){
		...
		for( i = Y->nmodes-4; i >= 0; i-- ){
			if( i == Y->nmodes-4){
				vdMul( Y->rank, &reordered_Factors[Y->nmodes-3][0], &reordered_Factors[Y->nmodes-2][0], &partial_Hadamards[Y->rank*i] );
			}
			else{
				vdMul( Y->rank, &reordered_Factors[i+1][0], &partial_Hadamards[Y->rank*(i+1)], &partial_Hadamards[Y->rank*i]);
			}
		}
		...
		for( i = 0; i < Y->dims_product/Y->dims[n]; i++ ){
			...
			*/
			vdMul( Y->rank, &reordered_Factors[0][Y->rank*indexers[0]], &partial_Hadamards[0], &C[Y->rank*i] );
			...
			update_Partial_Hadamards( Y, indexers, partial_Hadamards, reordered_Factors);
		}
...
/*
	...
	Variables -- 
		...
		B) matrix to compare against A
		nRows) the number of rows in A and B
...
void MTTKRP_RowMajor( tensor * T, double * K, double * C, long int rank, long int n){
	...
	if( n == 0 ){
		...
		nDim = T->dims[n];
		/*
			This degemm call performs the column wise matrix multiply between the first mode matricization of
			the tensor and the matrix K.
			Operation is X_1 x K
			Inputs -- 
			...
			6) ncols, the shared dimension, the number of columns in T->data and rows in K
			7) alpha
			8) T->data, the left matrix, first mode matricization of the tensor
			9) nDim, the leading dimension of T->data, the distance between columns of T->data
		...
		*/
		...
		//printf("Input args to cblas_dgemm function\nnDim=%d\nrank=%d\n,ncols=%d\n",*(T->dims+n-1), rank, ncols);
		// This does M*KR
		//cblas_dgemm( CblasRowMajor, CblasTrans, CblasNoTrans, nDim, rank, ncols, alpha, T->data, nDim, K, rank, beta, C, rank );
		// This does KR'*M'
		...
		//printf("Input args to cblas_dgemm function\nnDim=%d\nrank=%d\ncols=%d\nnmats=%d\ni=%d\n",nDim, rank, ncols, nmats, i);
			
			/*
				This dgemm call is more complex
				1) CblasColMajor - treat matrices as if they are in column major order
				2) CblasTrans - transpose the left matrix because we want it in row major ordering
				3) CblasNoTrans - still treat K as a column major matrix
			...
			*/	
			cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasNoTrans, nDim, rank, ncols, alpha, T->data + i*nDim*ncols, ncols, K+i*ncols*rank, rank, beta, C, rank );
		}
	}	// End of else
}
...
void MHada_RowMajor( ktensor * Y, double ** SYRKs, double * V, long int n ){
	...
	if( n == 0)	// if n==0 we do not want to use the 0th factor matrix so start at 1
		i = 1;
	else
		i = 0;
	
	for( c = 0; c < Y->nmodes - 1; c++, i++){	// c keeps track of the number of multiplies that should be performed
...
void do_SYRKs_RowMajor( ktensor * Y, double ** SYRKs ){
	...
	beta = 0.0;
	for( i = 0; i < Y->nmodes; i++ ){
		cblas_dsyrk( CblasRowMajor, CblasUpper, CblasTrans, Y->rank, Y->dims[i], alpha, Y->factors[i], Y->rank, beta, SYRKs[i], Y->rank);
	}
...
/*
	...
	Variables --
		Y) the ktensor
		n) the factor matrix to normalize
		i) general purpose iterator, moves over the rank of the nth factor matrix of Y
...
/*
	...
	Takes in a ktensor Y whose factor matrices are stored in row major ordering and
	normalizes them column-wise saving the weights in the Y->lambdas vector.
*/
void normalize_Ktensor_RowMajor( ktensor * Y ){
	...
	for( i = 0; i < Y->nmodes; i++ ){		// loop over the factor matrices
		for( j = 0; j < Y->rank; j++ ){		// loop over the columns of the factor matrices
			if( i == 0){
				Y->lambdas[j] = cblas_dnrm2( Y->dims[i], Y->factors[i] + j, Y->rank );
				if( Y->lambdas[j] == 0.0 ){
					exit(-8);
				}
				cblas_dscal( Y->dims[i], 1.0/Y->lambdas[j], Y->factors[i] + j, Y->rank );
			}
			else{
...
/*
	Full_nMode_Matricization_RowMajor();
		Description -- creates the full nmode matricization of a tensor from the factor matrices
		of a Ktensor.
...
void Full_nMode_Matricization_RowMajor( tensor * T, ktensor * Y, long int n ){
	...
	KR = (double *)malloc( sizeof(double) * (Y->dims_product/Y->dims[n]) * Y->rank  );

	Multi_KR_RowMajor( Y, KR, n );

	alpha = 1.0;
	beta = 0.0;
	cblas_dgemm( CblasColMajor, CblasTrans, CblasNoTrans, Y->dims[n], (Y->dims_product/Y->dims[n]), Y->rank, alpha, Y->factors[n], Y->rank, KR, Y->rank, beta,  T->data, Y->dims[n] );
...
double approximation_Error( double * X, double * KR, ktensor * Y, long int n ){
	...
	cblas_dgemm( CblasColMajor, CblasTrans, CblasNoTrans, Y->dims[n], (Y->dims_product/Y->dims[n]), Y->rank, alpha, Y->factors[n], Y->rank, KR, Y->rank, beta, X, Y->dims[n] );
...
double CP_ALS_efficient_error_computation( ktensor * Y, long int n, double * MTTKRP, double * V, double * S, double tensor_norm ){
	...
	cblas_dsyrk( CblasRowMajor, CblasUpper, CblasTrans, Y->rank, Y->dims[n], 1.0, Y->factors[n], Y->rank, 0.0, S, Y->rank );

	Upper_Hadamard_RowMajor(Y->rank, Y->rank, V, S, V);

	cblas_dsymv(CblasRowMajor, CblasUpper, Y->rank, 1.0, V, Y->rank, Y->lambdas, 1, 0.0, lambda_array, 1);
...
double CP_ALS_naive_error_computation( tensor * T, ktensor * Y, double * X, double * KRP, long int n ){
	...
	cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasTrans, Y->dims[n], (Y->dims_product/Y->dims[n]), Y->rank, 1.0, Y->factors[n], Y->rank, KRP, Y->rank, -1.0, X, Y->dims_product/Y->dims[n] );
...
void dims_check( long int * dims, long int length){
	...
	*/
	...
	for( i = 0; i < length; i++ ){
		if( dims[i] < 1 ){ printf("From dims_check():: dims[%d] < 1, Exit(-3)", i); exit(-3); }
	}
}
...
/*
	...
	Computes the factor matrix indeces for a given row in a khatri rao product.
...
/*
	Gen_Tensor();
	This function generates a Ktensor using uniformily random numbers. Given a Rank,
	a number of dimensions, and the sizes of those dimensiosn random factor matrices
	are generated and then a full tensor is created from those.
...
void Gen_Tensor( ktensor * Y, tensor * T, long int R, long int N, long int * D, double noise ){
	...
	*/
	...
	Y->dims_product = 1;
	for( i = 0; i < Y->nmodes; i++ ){
		Y->factors[i]	 = (double*)malloc(sizeof(double)*Y->rank*Y->dims[i]);
		Y->dims_product *= Y->dims[i];
		for( j = 0; j < Y->rank*Y->dims[i]; j++){
			Y->factors[i][j] = (((double)rand()/(double)RAND_MAX) * 2) - 1;
		}
	}
	Y->lambdas = (double*)malloc(sizeof(double)*Y->rank);
...
void parallel_Multi_KR_RowMajor( ktensor * Y, double * C, long int start, long int end){
	if( Y->nmodes == 2 ){
		...
		*/	
		...
		// Initialize the partial_Hadamards to the starting values, i indexes the partial_Hadamards
		// Hadamard the last two factor matrices and stores them in the last partial hadamard
		vdMul( Y->rank, &Y->factors[nmodes-2][Y->rank*indexers[nmodes-2]], &Y->factors[nmodes-1][Y->rank*indexers[nmodes-1]], &partial_Hadamards[Y->rank*(nmodes-3)] );

		// For each other partial Hadamard multiply the previous partial Hadamard with a row of a factor matrix
		for( i = nmodes-4; i >= 0; i-- )
			vdMul( Y->rank, &Y->factors[i+1][Y->rank*indexers[i+1]], &partial_Hadamards[Y->rank*(i+1)], &partial_Hadamards[Y->rank*i]);
	
		// Iterate over the rows of the KRP matrix
		for( i = start; i < end; i++ ){
			/*
				compute the current row using vd mul, multiply first partial hadamard with row of first factor matrix
				1) length of a row
				2) always some row of the 0th factor matrix
				3) always the first partial_hadamard
				4) the current row the KR matrix
			...
			*/
			vdMul( Y->rank, &Y->factors[0][Y->rank*indexers[0]], &partial_Hadamards[0], &C[Y->rank*(i-start)] );
			...
			for(; (c < end) && (j < Y->dims[0]); j++ ){
				/*
					...
					4) pointer to output location
						-i*Y->rank*Y->dims[0], the size of the right matrix times i
						-j*Y->rank, the length of a row times j
				*/
				vdMul( Y->rank, &Y->factors[1][i*Y->rank], &Y->factors[0][j*Y->rank], &C[(c-start)*Y->rank] );
				c++;
			}
		}
	free(indexers);
...
void parallel_update_Partial_Hadamards( ktensor * Y, long int * indexers, double * partial_Hadamards ){
	...
	if( update_from == 0 ){
		...
		/*
			If update_from is the second to last index or the last index, 
			we have to update all of the partials, including combining the last two factor matrices seperately
		*/
		/*
			call vdMul to get the hadamard product of the last two factor matrices
			1) length of the vectors
...
void process_inputs( long int argc, char *argv[], tensor_inputs * inputs ){
	...
	if( inputs->num_threads < 1 ){
		printf("From process_inputs(), num_threads = %d, num_threads must be at least 1\nExit\n",inputs->num_threads);
		exit(-1);
	}
	if( inputs->rank < 1 ){
