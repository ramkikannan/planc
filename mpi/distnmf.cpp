...
class DistNMFDriver {
 private:
    ...
    void callDistNMF1D() {
        ...
        FMAT W
            = arma::randu<FMAT>(this->m_globalm / mpicomm.size(), this->m_k);
        FMAT H
            = arma::randu<FMAT>(this->m_globaln / mpicomm.size(), this->m_k);
        sleep(10);
    ...
    void callDistNMF2D() {
        ...
        Pacoss_Communicator<float>::loadDistributedDimPart(
            dim_part_file_name.c_str(),
            dim_part);
        Pacoss_Communicator<float> *rowcomm = new Pacoss_Communicator<float>(
            MPI_COMM_WORLD,
            ss._idx[0],
            dim_part[0]);
        Pacoss_Communicator<float> *colcomm = new Pacoss_Communicator<float>(
            MPI_COMM_WORLD,
            ss._idx[1],
            dim_part[1]);
        ...
        SP_FMAT A(locations, values); A.resize(
...
# ifdef BUILD_SPARSE
        ...
        if (m_nmfalgo == ANLSBPP2D) {
            DistHALS<SP_FMAT> lrinitializer(A,
                                            W,
                                            H,
                                            mpicomm,
                                            this->m_num_k_blocks);
