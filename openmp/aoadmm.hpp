...
#ifndef OPENMP_AOADMM_HPP_
#define OPENMP_AOADMM_HPP_
template <class T>
    ...
    void computeNMF() {
        ...
        while (currentIteration < this->num_iterations()) {
            ...
            for (int i = 0; i < admm_iter && !stop_iter; i++) {
                H0 = this->H;
                tempHtaux = arma::solve(arma::trimatl(L), WtA + (beta * (this->H.t() + V.t())));
                Htaux = arma::solve(arma::trimatu(L.t()), tempHtaux);
                
                this->H = Htaux.t();
                fixNumericalError<MAT >(&(this->H), EPSILON_1EMINUS16);
                this->H = this->H - V;
                this->H.for_each([](MAT::elem_type& val) {
                    val = val > 0.0 ? val : 0.0;
                } );
                V = V + this->H - Htaux.t();
            ...
            for (int i = 0; i < admm_iter && !stop_iter; i++) {
                W0 = this->W;
                tempWtaux = arma::solve(arma::trimatl(L), AH.t() + alpha * (this->W.t() + U.t()));
                Wtaux = arma::solve(arma::trimatu(L.t()), tempWtaux);
                
                this->W = Wtaux.t();
                fixNumericalError<MAT >(&(this->W), EPSILON_1EMINUS16);
                this->W = this->W - U;
                this->W.for_each([](MAT::elem_type& val) {
                    val = val > 0.0 ? val : 0.0;
                } );
            ...
            INFO << "Completed it = " << currentIteration << " AOADMMERR="
