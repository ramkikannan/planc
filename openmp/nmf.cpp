...
void NMFDriver(int k, uword m, uword n, std::string AfileName, std::string WinitFileName, std::string HinitFileName, std::string WfileName, std::string HfileName, int numIt)
{
#ifdef BUILD_SPARSE
    sp_fmat A;
    uword nnz;
    ...
    if (!WinitFileName.empty())
    {
        ...
        nmfAlgorithm.setNumIterations(numIt);
...
void testSmall()
{
    ...
    INFO << "Computed Fast NMF with multiple RHS!!!:" << norm((A - bppnmf2.getLeftLowRankFactor()*bppnmf2.getRightLowRankFactor().t()), "fro") << endl;
    INFO << "time taken:" << (t2 - t1) << endl;
...
void testLowRank()
{
    ...
    W = W.rows(0, 50000);
    H = H.rows(0, 100000);
    INFO << "compled loading the matrices W.m=" << W.n_rows << " W.n=" << W.n_cols << " H.m=" << H.n_rows << " H.n=" << H.n_cols  << endl;
    A = W * H.t();
    ...
    INFO << "Computed Fast NMF with multiple RHS!!!:" << norm((A - bppnmf2.getLeftLowRankFactor()*bppnmf2.getRightLowRankFactor().t()), "fro") << endl;
    INFO << "time taken:" << (t2 - t1) << endl;
    bppnmf2.getLeftLowRankFactor().save("/scratch/ramki/datasets/xdata/bpp/w_lr_bpp_output.csv", raw_ascii);
    bppnmf2.getRightLowRankFactor().save("/scratch/ramki/datasets/xdata/bpp/h_lr_bpp_output.csv", raw_ascii);
}
#endif
void print_usage()
{
...
void parseCommandLineandCallNMF(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "a:h:i:k:m:n:t:w:", nmfopts, &long_index )) != -1)
    {
        switch (opt)
        {
        case 'a' :        
    ...
    case MU_NMF:
#ifdef BUILD_SPARSE
        NMFDriver<MUNMF<sp_fmat> >(lowRank, m, n, AfileName, WInitfileName, HInitfileName, WfileName, HfileName, numIt);
#else
        NMFDriver<MUNMF<fmat> >(lowRank, m, n, AfileName, WInitfileName, HInitfileName, WfileName, HfileName, numIt);
#endif
        break;
    case HALS_NMF:
#ifdef BUILD_SPARSE
        NMFDriver<HALSNMF<sp_fmat> >(lowRank, m, n, AfileName, WInitfileName, HInitfileName, WfileName, HfileName, numIt);
#else
        NMFDriver<HALSNMF<fmat> >(lowRank, m, n, AfileName, WInitfileName, HInitfileName, WfileName, HfileName, numIt);
#endif
        break;
    case BPP_NMF:
#ifdef BUILD_SPARSE
        NMFDriver<BPPNMF<sp_fmat> >(lowRank, m, n, AfileName, WInitfileName, HInitfileName, WfileName, HfileName, numIt);
#else
        NMFDriver<BPPNMF<fmat> >(lowRank, m, n, AfileName, WInitfileName, HInitfileName, WfileName, HfileName, numIt);
#endif
        break;
    }
}
